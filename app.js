...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadInitialData() {
        if (!user) return;
        try {
            // Carrega apenas lideranças (geralmente poucos registros) para popular selects
            // NOTA: usa sbService enquanto RLS não estiver configurado no Supabase
            const { data: leadersData, error: leadersError } = await sbService
                .from('cidadaos')
                .select('id, name, type')
                .eq('type', 'Liderança')
    ...
    async function loadCidadaosPage(reset = false) {
        ...
        // NOTA: usa sbService enquanto RLS não estiver configurado no Supabase
        let query = sbService.from('cidadaos').select('*', { count: 'exact' });
    ...
    try {
        ...
        closeCidadaoModal();
        // PERFORMANCE: recarrega só a página atual, sem baixar tudo
        await renderCidadaos();
        // Atualiza os selects de lideranças se o tipo mudou
        if (cidadaoType.value === 'Liderança') {
            const { data } = await sbService.from('cidadaos').select('id, name, type').eq('type', 'Liderança').order('name');
    ...
    async function handleDemandaFormSubmit(e) {
        ...
        try {
            ...
            closeDemandaModal();
            // PERFORMANCE: busca só as demandas, sem recarregar cidadãos
            // Recarrega com JOIN para ter nome do solicitante
            const { data: novasDemandas } = await sbService
                .from('demandas')
                .select('*, cidadao:cidadaos(id, name, leader)')
                .order('created_at', { ascending: false });
    ...
    // getFilteredCidadaos removida — filtragem agora é feita no servidor via loadCidadaosPage
    ...
    async function updateDashboard() {
        // PERFORMANCE: usa o total real do servidor, não o length do array local (que é só 1 página)
        const totalEl = document.getElementById('dashboard-total-cidadaos');
        if (totalCidadaosCount > 0) {
            totalEl.textContent = totalCidadaosCount;
        } else {
            // Primeira carga — busca count sem baixar dados
            const { count } = await sbService.from('cidadaos').select('*', { count: 'exact', head: true });
    ...
    async function updateCidadaosPorBairroChart() {
        ...
        try {
            const { data, error } = await sb.rpc('count_by_bairro').limit(10);
    ...
    async function openCidadaoModal(cidadaoId = null) {
        ...
        if (cidadaoId) {
            ...
            // Tenta cache local primeiro; se não estiver (paginação), busca no servidor
            let cidadao = allCidadaos.find(c => c.id === cidadaoId);
            if (!cidadao) {
                const { data, error } = await sbService
                    .from('cidadaos').select('*').eq('id', cidadaoId).single();
                if (!error && data) cidadao = data;
            }
            if (cidadao) {
        ...
        if (cidadaoId) {
            ...
            if (!cidadao) {
                const { data } = await sbService
                    .from('cidadaos').select('id, name').eq('id', cidadaoId).single();
                if (data) cidadao = data;
            }
            if (cidadao) {
    ...
    async function requestDelete(itemId, type) {
        itemToDelete = { id: itemId, type: type };
        const modal = document.getElementById('confirmation-modal');
        const title = document.getElementById('confirmation-title');
        const message = document.getElementById('confirmation-message');
        if (type === 'cidadao') {
            let cidadao = allCidadaos.find(c => c.id === itemId);
            if (!cidadao) {
                const { data } = await sbService
                    .from('cidadaos').select('name').eq('id', itemId).single();
                if (data) cidadao = data;
            }
            title.textContent = 'Excluir Cidadão';
            message.textContent = `Tem a certeza que quer excluir "${cidadao ? cidadao.name : 'este cidadão'}"?`;
        } else if (type === 'demanda') {
...
function closeMapModal() {
    ...
    async function generatePrintReport() {
        ...
        const s = serverSearchState;
        let query = sbService.from('cidadaos').select('name, type, phone, whatsapp, email, logradouro, numero, complemento, bairro, cidade, estado, cep');
