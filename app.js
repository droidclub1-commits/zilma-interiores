...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadInitialData() {
        ...
        try {
            ...
            // Carrega demandas (geralmente muito menos que cidadãos)
    ...
    async function handleDemandaFormSubmit(e) {
        ...
        try {
            ...
            const { data: novasDemandas } = await sb.from('demandas').select('*').order('created_at', { ascending: false });
    ...
    async function updateAniversariantes() {
        ...
        try {
            ...
            const { data, error } = await sbService
                ...
                .select('id, name, dob')
                .not('dob', 'is', null)
                .gte('dob', `0001-${mesStr}-01`)   // dob >= qualquer-ano-mês-01
                .lte('dob', `9999-${mesStr}-31`)   // dob <= qualquer-ano-mês-31
                .order('dob', { ascending: true })
                .limit(200);
            if (error) throw error;

            // Filtra no cliente o mês exato (garante precisão independente do banco)
            const aniversariantes = (data || []).filter(c => {
                if (!c.dob) return false;
                const dobMes = parseInt(c.dob.split('-')[1], 10);
                return dobMes === mes;
            });
            aniversariantes.sort((a, b) => parseInt(a.dob.split('-')[2]) - parseInt(b.dob.split('-')[2]));
            listEl.innerHTML = '';
    ...
    async function updateCidadaosPorTipoChart() {
        ...
        if (!ctx) return;
        try {
            // PERFORMANCE: conta por tipo no servidor sem baixar todos os registros
            const tipos = ['Cidadão Comum', 'Liderança', 'Vereador', 'Apoiador', 'Outro'];
            const counts = await Promise.all(tipos.map(t =>
                sbService.from('cidadaos').select('*', { count: 'exact', head: true }).eq('type', t)
            ));
            const labels = [], values = [];
            counts.forEach((res, i) => { if ((res.count || 0) > 0) { labels.push(tipos[i]); values.push(res.count); } });
            if (cidadaosChart) cidadaosChart.destroy();
            cidadaosChart = new Chart(ctx, {
                type: 'pie',
                data: { labels, datasets: [{ label: 'Cidadãos por Tipo', data: values, backgroundColor: ['#3B82F6', '#8B5CF6', '#10B981', '#F59E0B', '#6B7280'] }] },
